...
class PostModel:
    ...
    def find_by_id(self, post_id: int) -> Optional[Post]:
        ...
        return next((post for post in self._posts if post.id == post_id and post.del_yn == "N"), None)

    def get_all(self) -> List[Post]:
        """전체 게시글 목록 조회 (삭제되지 않은 것만)"""
        return [post for post in self._posts if post.del_yn == "N"]

    def update(self, post_id: int, title: Optional[str] = None, content: Optional[str] = None,
               img_url: Optional[str] = None) -> Optional[Post]:
        """게시글 수정"""
    ...
    def get_by_author(self, author_id: int) -> List[Post]:
        ...
        return [post for post in self._posts if post.author_id == author_id and post.del_yn == "N"]
